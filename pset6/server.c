...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            // give error if the request-line doesn't contain \r\n
            if (needle == NULL)
            {
                error(400);
                continue;
            }
            // give error if the request-line is too long
            ...
            //create variable line to store request-line   
            char line[needle - haystack + 2 + 1];
            //copy request-line to line 
            strncpy(line, haystack, needle - haystack + 2);
            //manually add NULL at the end of the line 
            ...
            char * pch; char* pch1; char* pch2; char* pch3;
            pch=strchr(line,' ');
            int counter = 1;
            while (pch!=NULL)
            {
            ...
            //declare and extract method from request-line
            char method[pch1 - line + 1];
            strncpy(method, line, pch1 - line);
            method[pch1 - line] = '\0';

            //declare and extract request-target from request-line
            char request_target[pch2 - pch1];
            strncpy(request_target, pch1+1, pch2 - pch1 - 1);
            request_target[pch2 - pch1 -1] = '\0';
            ...
            {
                strncpy(abspath, request_target, strchr(request_target,'?') - request_target);
                abspath[strchr(request_target, '?') - request_target + 1] = '\0';
            }
            else if (strchr(request_target, '?') == NULL)
            {
                strcpy(abspath, request_target);
            }
            ...
            pch3= strstr(pch,"\r\n");
            char http_version[pch3 - pch + 1];
            strncpy(http_version, pch, pch3 - pch);
            http_version[pch3 - pch] = '\0';
            ...
            if((strchr(request_target, '?') == NULL) || ((strchr(request_target,'?')+1) == NULL))
            {
                strcpy(query, "");
            }
            else
            {
                char* needle1;
                char* end_target;
                needle1 = strchr(request_target, '?') + 1;
                end_target = strchr(request_target,'\0');
                strncpy(query, needle1, end_target - needle1);
                query[end_target - needle1] = '\0';
              }
            ...
            int extension_length = strchr(path, '\0') - strchr(path, '.') - 1;

            char extension[extension_length + 1];

            strncpy(extension,strchr(path,'.') + 1,extension_length);
...
/**
 * Returns MIME type for supported s, else NULL.
 */
const char* lookup(const char* extension)
{
